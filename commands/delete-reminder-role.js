...
export default {
  data: new SlashCommandBuilder()
    .setName("delete-reminder-rol")
    .setDescription("Reminder komudunu kullanacak rolü sil."),

  async execute(interaction) {
      const settings = await getSettings(interaction.guild.id);
      if (!settings) {
        await saveSettings(interaction.guild.id, { reminderRole: "", permissionRole: "" });
      ...
      if (!hasAdmin) {
        const response = {
          content: "❌ delete-reminder-role Bu komutu kullanmak için yetkin yok!",
    ...
    try {
      ...
      await interaction.editReply({
        embeds: [
          new EmbedBuilder()
            .setColor("#4BB543")
            .setTitle("❌ Role başarıyla kaldırıldı.")
